...
import { C, Badge } from "../utils/ui.jsx";
  ...
  return (
    <>
      <div className="bento-span-full" style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: 0.6 }}>
        Adapter · Expérimenter
      </div>
      {experiments.map((exp, i) => {
        const isLast = i === experiments.length - 1;
        const cardClass = `bento-card ${isLast && experiments.length % 2 !== 0 ? "bento-span-full" : "bento-span-2"}`;
        return (
          <React.Fragment key={exp.label}>
            ...
            {exp.gate && !isLast ? (
              <div className="bento-span-full" style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0", fontSize: 11, color: C.muted }}>
